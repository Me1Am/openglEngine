...
struct Material {
	sampler2D diffuseTexture;	// Base texture
...
vec3 calcDirLight(DirLight light, vec3 normal, vec3 cameraDir, vec3 textureColor, vec3 diffuseTextureColor);
vec3 calcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 cameraDir, vec3 textureColor, vec3 diffuseTextureColor);
vec3 calcSpotlight(Spotlight light, vec3 normal, vec3 fragPos, vec3 cameraDir, vec3 textureColor, vec3 diffuseTextureColor);
...
void main() {
	...
	vec3 textureColor = vec3(texture(material.diffuseTexture, TexCoord));
	vec3 diffuseTextureColor = vec3(texture(material.specMap, TexCoord));

	// Add directional light calculation(only 1)
	result += calcDirLight(dirLight, normal, cameraDir, textureColor, diffuseTextureColor);

	// Add all point lights
	for(int i = 0; i < NUM_POINT_LIGHTS; i++) {
		if(pointLights[i].diffuse == 0.f) continue;
		result += calcPointLight(pointLights[i], normal, FragPos, cameraDir, textureColor, diffuseTextureColor);
	}
	// Add all spotlights
	for(int i = 0; i < NUM_SPOTLIGHTS; i++) {
		result += calcSpotlight(spotlights[i], normal, FragPos, cameraDir, textureColor, diffuseTextureColor);
	}
...
vec3 calcDirLight(DirLight light, vec3 normal, vec3 cameraDir, vec3 textureColor, vec3 diffuseTextureColor) {
	...
	vec3 specular = light.specular * spec * diffuseTextureColor;
